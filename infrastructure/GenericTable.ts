...
export class GenericTable {
  ...
  constructor(public stack: Stack, props: TableProps) {
    this.props = props;
    this.initalize();
  ...
  private addSecondaryIndexes() {
    if(this.props.scondaryIndex){
      for (const secondaryIndex of this.props.scondaryIndex) {
  ...
  private createSingleLambda(lambda: string): NodejsFunction {
    const lambdaId = `${this.props.tableName}-${lambda}`
    return new NodejsFunction(this.stack, lambdaId, {
      entry: (join(__dirname, '..', 'services', this.props.tableName, `${lambda}.ts`)),
      handler: 'handler',
